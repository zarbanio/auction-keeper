...
func NewClipperTakeService(
	...
	if err != nil {
		services.Logger.Fatal().Str("service", "take").Str("method", "NewClipperTakeService").Msg("err while instancing a clipper contract")
	}
...
func (s *ClipperTakeService) Start(ctx context.Context, minProfitPercentage, minLotZarValue, maxLotZarValue *big.Int, profitAddress common.Address) error {
	services.Logger.Info().Str("service", "take").Str("method", "Start").Msg("take Service is starting")

	currentTime, err := s.getCurrentTime(ctx)
	if err != nil {
		services.Logger.Error().Err(err).Str("service", "take").Str("method", "Start").Msg("error while getting current time")
		return err
	}
	...
	if err != nil {
		services.Logger.Error().Err(err).Str("service", "take").Str("method", "Start").Msg("error while getting the list of auction IDs")
		return err
	}
	...
	for id, auctionId := range auctionIds {
		...
		if err != nil {
			services.Logger.Error().Err(err).Str("service", "take").Str("method", "Start").Msg("error while getting an auction")
			return err
		}
		...
		if err != nil {
			...
			services.Logger.Error().Err(err).Str("service", "take").Str("method", "Start").Msg("error while getting the collateral price")
			continue
		}
		...
		if err != nil {
			...
			services.Logger.Error().Err(err).Str("service", "take").Str("method", "Start").Msg("error while getting the uniswapV3 proceeds")
			continue
		}
		...
		if debtToCover.Cmp(minUniV3Proceeds) <= 0 {
			...
			if err != nil {
				...
				services.Logger.Error().Err(err).Str("service", "take").Str("method", "Start").Msg("error while executing an auction")
				continue
			}
		} else {
...
func (s *ClipperTakeService) Run(take inputMethods.ClipperTake, minProfit *big.Int, profitAddr, gemJoinAdapter common.Address) error {
	...
	if err != nil {
		services.Logger.Error().Err(err).Str("service", "take").Str("method", "Run").Msg("error while getting the route from uniswapV3")
	...
	if err != nil {
		services.Logger.Error().Err(err).Str("service", "take").Str("method", "Run").Msg("error while getting the flashData")
		return fmt.Errorf(fmt.Sprintf("error in pack flash data: : %v", err))
	}

	opts, err := s.sender.GetTransactOpts()
	if err != nil {
		services.Logger.Error().Err(err).Str("service", "take").Str("method", "Run").Msg("error while getting a transaction opts")
		return err
	}

	tx, err := s.clipper.Take(opts, take.Auction_id, take.Amt, take.Max, take.Who, flashData)
	if err != nil {
		services.Logger.Error().Err(err).Str("service", "take").Str("method", "Run").Msg("error while calling the clipper take method")
		return err
	}
...
func (s *ClipperTakeService) Stop() {
	services.Logger.Panic().Str("service", "take").Str("method", "Stop").Msg("Implement me!")
}
...
func (s *ClipperTakeService) getCurrentTime(ctx context.Context) (uint64, error) {
	...
	if err != nil {
		services.Logger.Error().Err(err).Str("service", "take").Str("method", "getCurrentTime").Msg("error while getting the last block number")
		return 0, err
	}
	block, err := s.eth.BlockByNumber(context.Background(), big.NewInt(int64(blockNum)))
	if err != nil {
		services.Logger.Error().Err(err).Str("service", "take").Str("method", "getCurrentTime").Msg("error while getting the head")
		return 0, err
	}
