...
type CageHandler struct {
	addr string
	binding  *Zarjoin
	callback events.CallbackFn[ZarjoinCage]
}
...
func (h *CageHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(ZarjoinCage)
	if !ok {
		return errors.New("event type is not ZarjoinCage")
	}
...
func NewCageHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[ZarjoinCage]) events.Handler {
