...
type ExitHandler struct {
	addr string
	binding  *Zarjoin
	callback events.CallbackFn[ZarjoinExit]
}
...
func (h *ExitHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(ZarjoinExit)
	if !ok {
		return errors.New("event type is not ZarjoinExit")
	}
...
func NewExitHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[ZarjoinExit]) events.Handler {
