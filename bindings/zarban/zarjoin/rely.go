...
type RelyHandler struct {
	addr string
	binding  *Zarjoin
	callback events.CallbackFn[ZarjoinRely]
}
...
func (h *RelyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(ZarjoinRely)
	if !ok {
		return errors.New("event type is not ZarjoinRely")
	}
...
func NewRelyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[ZarjoinRely]) events.Handler {
