...
type DenyHandler struct {
	addr string
	binding  *Zarjoin
	callback events.CallbackFn[ZarjoinDeny]
}
...
func (h *DenyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(ZarjoinDeny)
	if !ok {
		return errors.New("event type is not ZarjoinDeny")
	}
...
func NewDenyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[ZarjoinDeny]) events.Handler {
