...
type RelyHandler struct {
	addr string
	binding  *Jug
	callback events.CallbackFn[JugRely]
}
...
func (h *RelyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(JugRely)
	if !ok {
		return errors.New("event type is not JugRely")
	}
...
func NewRelyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[JugRely]) events.Handler {
