...
type DenyHandler struct {
	addr string
	binding  *Spot
	callback events.CallbackFn[SpotDeny]
}
...
func (h *DenyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(SpotDeny)
	if !ok {
		return errors.New("event type is not SpotDeny")
	}
...
func NewDenyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[SpotDeny]) events.Handler {
