...
type File0Handler struct {
	addr string
	binding  *Spot
	callback events.CallbackFn[SpotFile0]
}
...
func (h *File0Handler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(SpotFile0)
	if !ok {
		return errors.New("event type is not SpotFile0")
	}
...
func NewFile0Handler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[SpotFile0]) events.Handler {
