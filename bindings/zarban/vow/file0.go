...
type File0Handler struct {
	addr string
	binding  *Vow
	callback events.CallbackFn[VowFile0]
}
...
func (h *File0Handler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(VowFile0)
	if !ok {
		return errors.New("event type is not VowFile0")
	}
...
func NewFile0Handler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[VowFile0]) events.Handler {
