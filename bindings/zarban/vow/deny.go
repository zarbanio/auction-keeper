...
type DenyHandler struct {
	addr string
	binding  *Vow
	callback events.CallbackFn[VowDeny]
}
...
func (h *DenyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(VowDeny)
	if !ok {
		return errors.New("event type is not VowDeny")
	}
...
func NewDenyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[VowDeny]) events.Handler {
