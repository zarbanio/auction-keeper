...
type RelyHandler struct {
	addr string
	binding  *Vow
	callback events.CallbackFn[VowRely]
}
...
func (h *RelyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(VowRely)
	if !ok {
		return errors.New("event type is not VowRely")
	}
...
func NewRelyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[VowRely]) events.Handler {
