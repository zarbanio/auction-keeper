...
type FileHandler struct {
	addr string
	binding  *Vow
	callback events.CallbackFn[VowFile]
}
...
func (h *FileHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(VowFile)
	if !ok {
		return errors.New("event type is not VowFile")
	}
...
func NewFileHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[VowFile]) events.Handler {
