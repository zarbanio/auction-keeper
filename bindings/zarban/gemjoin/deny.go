...
type DenyHandler struct {
	addr string
	binding  *Gemjoin
	callback events.CallbackFn[GemjoinDeny]
}
...
func (h *DenyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(GemjoinDeny)
	if !ok {
		return errors.New("event type is not GemjoinDeny")
	}
...
func NewDenyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[GemjoinDeny]) events.Handler {
