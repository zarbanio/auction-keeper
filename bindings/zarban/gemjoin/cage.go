...
type CageHandler struct {
	addr string
	binding  *Gemjoin
	callback events.CallbackFn[GemjoinCage]
}
...
func (h *CageHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(GemjoinCage)
	if !ok {
		return errors.New("event type is not GemjoinCage")
	}
...
func NewCageHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[GemjoinCage]) events.Handler {
