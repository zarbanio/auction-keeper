...
type RelyHandler struct {
	addr string
	binding  *Gemjoin
	callback events.CallbackFn[GemjoinRely]
}
...
func (h *RelyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(GemjoinRely)
	if !ok {
		return errors.New("event type is not GemjoinRely")
	}
...
func NewRelyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[GemjoinRely]) events.Handler {
