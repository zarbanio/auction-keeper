...
type JoinHandler struct {
	addr string
	binding  *Gemjoin
	callback events.CallbackFn[GemjoinJoin]
}
...
func (h *JoinHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(GemjoinJoin)
	if !ok {
		return errors.New("event type is not GemjoinJoin")
	}
...
func NewJoinHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[GemjoinJoin]) events.Handler {
