...
type DenyHandler struct {
	addr string
	binding  *Ilkregistry
	callback events.CallbackFn[IlkregistryDeny]
}
...
func (h *DenyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(IlkregistryDeny)
	if !ok {
		return errors.New("event type is not IlkregistryDeny")
	}
...
func NewDenyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[IlkregistryDeny]) events.Handler {
