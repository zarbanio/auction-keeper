...
type File1Handler struct {
	addr string
	binding  *Ilkregistry
	callback events.CallbackFn[IlkregistryFile1]
}
...
func (h *File1Handler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(IlkregistryFile1)
	if !ok {
		return errors.New("event type is not IlkregistryFile1")
	}
...
func NewFile1Handler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[IlkregistryFile1]) events.Handler {
