...
type DenyHandler struct {
	addr string
	binding  *Median
	callback events.CallbackFn[MedianDeny]
}
...
func (h *DenyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(MedianDeny)
	if !ok {
		return errors.New("event type is not MedianDeny")
	}
...
func NewDenyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[MedianDeny]) events.Handler {
