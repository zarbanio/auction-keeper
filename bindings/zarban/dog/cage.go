...
type CageHandler struct {
	addr string
	binding  *Dog
	callback events.CallbackFn[DogCage]
}
...
func (h *CageHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(DogCage)
	if !ok {
		return errors.New("event type is not DogCage")
	}
...
func NewCageHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[DogCage]) events.Handler {
