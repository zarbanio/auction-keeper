...
type RelyHandler struct {
	addr string
	binding  *Dog
	callback events.CallbackFn[DogRely]
}
...
func (h *RelyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(DogRely)
	if !ok {
		return errors.New("event type is not DogRely")
	}
...
func NewRelyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[DogRely]) events.Handler {
