...
type DenyHandler struct {
	addr string
	binding  *Dog
	callback events.CallbackFn[DogDeny]
}
...
func (h *DenyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(DogDeny)
	if !ok {
		return errors.New("event type is not DogDeny")
	}
...
func NewDenyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[DogDeny]) events.Handler {
