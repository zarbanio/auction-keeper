...
type FileHandler struct {
	addr string
	binding  *Dog
	callback events.CallbackFn[DogFile]
}
...
func (h *FileHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(DogFile)
	if !ok {
		return errors.New("event type is not DogFile")
	}
...
func NewFileHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[DogFile]) events.Handler {
