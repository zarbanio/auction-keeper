...
type DenyHandler struct {
	addr string
	binding  *Flopper
	callback events.CallbackFn[FlopperDeny]
}
...
func (h *DenyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(FlopperDeny)
	if !ok {
		return errors.New("event type is not FlopperDeny")
	}
...
func NewDenyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[FlopperDeny]) events.Handler {
