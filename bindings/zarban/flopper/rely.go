...
type RelyHandler struct {
	addr string
	binding  *Flopper
	callback events.CallbackFn[FlopperRely]
}
...
func (h *RelyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(FlopperRely)
	if !ok {
		return errors.New("event type is not FlopperRely")
	}
...
func NewRelyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[FlopperRely]) events.Handler {
