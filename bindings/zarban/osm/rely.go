...
type RelyHandler struct {
	addr string
	binding  *Osm
	callback events.CallbackFn[OsmRely]
}
...
func (h *RelyHandler) HandleEvent(log eth.Log, event interface{}) error {
	e, ok := event.(OsmRely)
	if !ok {
		return errors.New("event type is not OsmRely")
	}
...
func NewRelyHandler(addr common.Address, eth *ethclient.Client, callback events.CallbackFn[OsmRely]) events.Handler {
