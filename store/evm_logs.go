...
func (l *evmLogModel) toDomain() *eth.Log {
	return &eth.Log{
		Id:        l.Id,
		Log:       *l.toRaw(),
...
func (l *evmLogModel) toRaw() *types.Log {
...
func (p postgres) CreateLog(ctx context.Context, log eth.Log) (uint64, error) {
	...
	err := p.conn.QueryRow(ctx, `
        INSERT INTO evm_logs (address, timestamp, block_number, tx_hash, tx_index, block_hash, index, data, topics)
